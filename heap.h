...
template <typename T> void Heap<T>::print() const {
    for (const T &t : heap) {
        std::cout << t << ", ";
    }
    std::cout << std::endl;
...
template <typename T> void Heap<T>::sift_up(size_t index) {
    if (index > 0) {
...
template <typename T> void Heap<T>::sift_down(size_t index) {
...
template <typename T> void Heap<T>::remove(size_t index) {
    if (index < heap.size()) {
...
template <typename T> bool Heap<T>::check_children(size_t index) {
    size_t left = index * 2 + 1;
    size_t right = index * 2 + 2;
...
template <typename T> T Heap<T>::pop_root() {
    T t = heap.front();
    remove(0);
    return t;
}
